...
module.exports = class InlineGitDiff {
  ...
  renderDiff(diff) {
    ...
    if (diff.lines.removed.length) {
      const editorInEditor = buildEditorForRemovedDiff(this.editor.getGrammar(), diff)
  ...
  copyRemovedText() {
    ...
    if (diff) {
      ...
      this.flashRange(editorInEditor, range, 800, {type: "line", class: "inline-git-diff-flash-copy"})
    }
  }
