...
class Hunk {
  ...
  getPointToInsertBlockDecoration(which) {
    if (this.kind === "added") {
      throw new Error("not allowed to call for 'added' kind hunk")
    }

    if (this.kind === "modified") {
      return new Point(which === "before" ? this.startRow : this.endRow, 0)
    } else {
      return new Point(which === "before" ? this.startRow + 1 : this.startRow, 0)
    }
  }
}
