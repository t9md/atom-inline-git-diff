...
module.exports = class GitDiffDetailsView extends View {
  ...
  display(hunk) {
    ...
    this.decorateRange(this.editor, [point, point], {type: "block", position: "after", item: diffView})
  }
  ...
  async withKeepingRelativeScrollPosition(fn) {
    ...
    const oldPixelTop = topPixelForBufferRow(this.editor, cursorRow)
    await fn()
    await this.editor.component.getNextUpdatePromise()
    const newPixelTop = topPixelForBufferRow(this.editor, cursorRow)
    const amountOfScroledPixels = newPixelTop - oldPixelTop
    if (amountOfScroledPixels) {
      const scrollTop = this.editor.element.getScrollTop() + amountOfScroledPixels
