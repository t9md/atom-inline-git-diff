...
class AtomGitDiffDetailsView extends View {
  ...
  updateCurrentRow() {
    const newCurrentRow =
      __guard__(__guard__(this.getActiveTextEditor(), x1 => x1.getCursorBufferPosition()), x => x.row) + 1
  ...
  notifyContentsModified() {
    ...
    this.diffDetailsDataManager.invalidate(
      this.repositoryForPath(this.editor.getPath()),
      this.editor.getPath(),
      this.editor.getText()
    )
  ...
  display(selectedHunk) {
    ...
    const classPostfix = atom.config.get("git-diff-details.enableSyntaxHighlighting") ? "highlighted" : "flat"
...
module.exports = AtomGitDiffDetailsView
