...
module.exports = class GitDiffDetailsView extends View {
  ...
  decorateLines(editor, start, end, className) {
    const marker = decorateRange(editor, [[start, 0], [end, 0]], {type: "line", class: className})
  ...
  display(hunk) {
    ...
    const classNameFor = kind => `git-diff-details-${kind}-${flatOrHighlight}`

    // host editor
    {
      const className = classNameFor("new")
      if (hunk.kind === "m") {
    ...
    {
      const className = classNameFor("old")
