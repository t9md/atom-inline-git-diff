...
module.exports = class GitDiffDetailsView {
  ...
  revert() {
    ...
    if (hunk) {
      ...
      if (hunk.kind !== "added") {
        this.getRemovedDiffForHunk(hunk).destroy()
      }
      hunk.revert(this.editor)
      this.editor.setCursorBufferPosition(marker.getHeadBufferPosition())
    }
  }

  getRemovedDiffForHunk(hunk) {
    return this.removedDiffs.find(removedDiff => removedDiff.hunk === hunk)
  }
