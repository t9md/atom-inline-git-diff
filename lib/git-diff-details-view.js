...
const {Range, Point, CompositeDisposable} = require("atom")
...
module.exports = class AtomGitDiffDetailsView extends View {
  ...
  updateCurrentRow() {
    const currentRow = this.editor.getCursorBufferPosition().row + 1
  ...
  updateDiffDetailsDisplay() {
    if (this.showDiffDetails) {
      ...
      this.previousSelectedHunk = selectedHunk // FIXME
    }
    this.destroyDecoration()
  }
