...
module.exports = class GitDiffDetailsView extends View {
  ...
  display(hunk) {
    ...
    const classNameFor = (kind, flatOrHighlighted) => `git-diff-details-${kind}-${flatOrHighlighted}`
    // host editor
    {
      const className = classNameFor("new", getConfig("useFlatColorForAddedLines") ? "flat" : "highlighted")
    ...
    {
      const className = classNameFor("old", getConfig("useFlatColorForRemovedLines") ? "flat" : "highlighted")
