...
module.exports = class GitDiffDetailsView extends View {
  ...
  display(hunk) {
    ...
    // host editor
    {
      ...
      if (hunk.kind === "m") {
        if (getConfig("showWordDiffs"))
          this.decorateWords(this.editor, hunk.start - 1, hunk.newRelativeChanges, className, "added")
      }
    }
    ...
    if (getConfig("showWordDiffs")) {
      this.decorateWords(diffView.editor, 0, hunk.oldRelativeChanges, "git-diff-details-removed", "removed")
    }
