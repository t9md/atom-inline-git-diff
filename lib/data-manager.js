 ...
 */
let DiffDetailsDataManager;
const JsDiff = require('diff');

module.exports = (DiffDetailsDataManager = class DiffDetailsDataManager {
  constructor() {
    this.invalidate();
  }
  ...
  updateLineDiffDetails() {
    if ((this.lineDiffDetails == null) || this.lineDiffDetailsInvalidated) {
      ...
      if (this.lineDiffDetails) { this.prepareWordDiffs(this.lineDiffDetails); }
    }
  ...
  prepareLineDiffDetails(repo, path, text) {
    ...
    if (rawLineDiffDetails == null) { return; }
      ...
      // process modifications and deletions only
        if ((oldLines !== 0) || !(newLines > 0)) {
          ...
          if (newLineNumber >= 0) {
            ...
            result.push(hunk.newString += line);
          } else {
            hunk.oldLines.push(line);
            result.push(hunk.oldString += line);
          }
        } else {
          result.push(undefined);
        }
  ...
  prepareWordDiffs(lineDiffDetails) {
    return (() => {
      const result = [];
      for (var hunk of lineDiffDetails) {
        if ((hunk.kind !== "m") || (hunk.newLines.length !== hunk.oldLines.length)) { continue; }
        hunk.newWords = [];
        hunk.oldWords = [];
        result.push((() => {
          const result1 = [];
          for (var i = 0, end = hunk.newLines.length; i < end; i++) {
            var oldCol;
            var newCol = (oldCol = 0);
            var diff = JsDiff.diffWordsWithSpace(hunk.oldLines[i], hunk.newLines[i]);
            result1.push((() => {
              const result2 = [];
              for (let word of diff) {
                ...
                if (word.added) {
                  ...
                  word.endCol = newCol;
                  result2.push(hunk.newWords.push(word));
                } else if (word.removed) {
                  word.changed = true;
                  word.startCol = oldCol;
                  oldCol += word.value.length;
                  word.endCol = oldCol;
                  result2.push(hunk.oldWords.push(word));
                } else {
                  newCol += word.value.length;
                  oldCol += word.value.length;
