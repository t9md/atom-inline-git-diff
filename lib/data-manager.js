...
function hunkContainsLine(hunk, lineNumber) {
  return hunk.start <= lineNumber && lineNumber <= hunk.end
}
...
module.exports = class DiffDetailsDataManager {
  ...
  getSelectedHunk(lineNumber) {
    if (!this.selectedHunk || this.selectedHunkInvalidated || !hunkContainsLine(this.selectedHunk, lineNumber)) {
      if (!this.lineDiffDetails || this.lineDiffDetailsInvalidated) {
      ...
      this.selectedHunk = hunks.find(hunk => hunkContainsLine(hunk, lineNumber))
    }
    this.selectedHunkInvalidated = false

    const {previousSelectedHunk, selectedHunk} = this
    const isSameHunk = previousSelectedHunk && selectedHunk === previousSelectedHunk
    this.previousSelectedHunk = selectedHunk
    return {selectedHunk, isSameHunk}
  }
